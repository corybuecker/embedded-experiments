...
static const struct bt_uuid_16 service_uuid =
    BT_UUID_INIT_16(SERVICE_UUID);
...
struct bt_gatt_service service =
    BT_GATT_SERVICE(service_gatt_attributes);
  ...
  while (true)
  {
    ...
    if (default_connection != NULL)
    {
                                ...
                                BT_GATT_CCC_NOTIFY))
      {
        ...
        if (err)
        {
          LOG_ERR("bt_gatt_notify failed (err %d)\n", err);
        }
      }
    }
  }
...
int main(void)
{
  ...
  if (err)
  {
    LOG_ERR("could not start advertising");
    return err;
  }
